...
@NoArgsConstructor
@Entity
@Table(name = "project_schedule_job")
@SQLDelete(sql = "update project_schedule_job set is_deleted = 1 where job_id = ? and project_id = ?")
@SQLRestriction("is_deleted = 0")
@NamedEntityGraphs(
        @NamedEntityGraph(
                name = "project_schedule_job.all_task",
                attributeNodes = {
                        @NamedAttributeNode(value = "tasks")
                }
        )
)
public class ProjectScheduleJobDO extends BaseAggregationRoot<ProjectScheduleJobDO> {
    ...
    public static ProjectJobDO convertToProjectJobDO(ProjectScheduleJobDO projectScheduleJobDO) {
        ProjectJobDO projectJobDO = new ProjectJobDO();
        BeanUtils.copyProperties(projectScheduleJobDO, projectJobDO);
        projectJobDO.setTasks(projectScheduleJobDO.getTasks());
        projectJobDO.setStatus(projectScheduleJobDO.getStatus());
        projectJobDO.setUpk(new ProjectJobDO.UPK(projectScheduleJobDO.getUpk().projectId, projectScheduleJobDO.getUpk().jobId));
        return projectJobDO;
    }

    public static ProjectScheduleJobDO convertFromProjectJobDO(ProjectJobDO job) {
        ProjectScheduleJobDO projectScheduleJobDO = new ProjectScheduleJobDO();
        BeanUtils.copyProperties(job, projectScheduleJobDO);
        projectScheduleJobDO.setUpk(new ProjectScheduleJobDO.UPK(job.getUpk().getProjectId(), job.getUpk().getJobId()));
    ...
    public static class UPK implements Serializable {
        ...
        public boolean equals(Object o) {
            ...
            ProjectScheduleJobDO.UPK that = (ProjectScheduleJobDO.UPK) o;
    ...
    public static abstract class AbstractEvent {
        @JsonIgnore
        private ProjectScheduleJobDO source;
    ...
    public static class TaskStatusTransformEvent extends AbstractEvent {
        ...
        private TaskStatusTransformEvent(ProjectScheduleJobDO source) {
            super(source);
        }
         ...
         */
        public static TaskStatusTransformEvent of(ProjectScheduleJobDO source, String taskId, GraphNodeTaskStatus fromStatus,
