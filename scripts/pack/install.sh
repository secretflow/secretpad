...
function init_images_from_files() {
	for file in images/*; do
		if [ -f "$file" ]; then
			...
			elif [[ $someimage == *serving-anolis8* ]]; then
				SECRETFLOW_SERVING_IMAGE=$someimage
			fi
		fi
	done
}
...
function start_master() {
	...
	applySfServingAppimage ${USER}-kuscia-master
	loadSfServingImage2Container ${USER}-kuscia-lite-alice
	loadSfServingImage2Container ${USER}-kuscia-lite-bob
}

function applySfServingAppimage() {
	local container_id=$1
	echo "apply sf serving appimage"
	docker run --rm --entrypoint /bin/bash -v $(pwd):/tmp/secretpad $SECRETPAD_IMAGE -c 'cp -R /app/scripts/pack/serving/sf-serving.yaml /tmp/secretpad/'
	if [[ $SECRETFLOW_SERVING_IMAGE == *aliyuncs* ]]; then
		echo "SECRETFLOW_SERVING_IMAGE $SECRETFLOW_SERVING_IMAGE"
	else
		SECRETFLOW_SERVING_IMAGE="docker.io/"$SECRETFLOW_SERVING_IMAGE
		echo "SECRETFLOW_SERVING_IMAGE $SECRETFLOW_SERVING_IMAGE"
	fi
	echo "echo SECRETFLOW_SERVING_IMAGE $SECRETFLOW_SERVING_IMAGE "
	...
	echo "echo TEEAPPS_SIM_IMAGE_NAME $SECRETFLOW_SERVING_IMAGE "
	...
	echo "echo docker cp sf-serving.yaml  $container_id:/home/kuscia"
	docker cp sf-serving-0.yaml $container_id:/home/kuscia
	echo "echo docker exec -it $container_id kubectl apply -f sf-serving.yam"
...
function start_lite() {
	...
	loadSfServingImage2Container "$domain_ctr"
}
...
function start_kuscia() {
	...
	applySfServingAppimage "${USER}-kuscia-autonomy-${domain_id}"
	loadSfServingImage2Container "${USER}-kuscia-autonomy-${domain_id}"
}
...
export SECRETFLOW_SERVING_IMAGE=$SECRETFLOW_SERVING_IMAGE
echo -e "SECRETFLOW_SERVING_IMAGE=$SECRETFLOW_SERVING_IMAGE"
